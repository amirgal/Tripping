...
$("#map").on("click", "#editedSaveSpotBtn", function() {
  if (
    $("#edited-spot-name-input").val().length == 0 ||
    $("#edited-spot-comment-input").val().length == 0
  ) {
    alert("Spot name and spot description are required!");
  } else {
...
$("#map").on("click", ".edit-spot", function() {
  ...
  const tripName = $(this)
    .closest(".info-window")
    .data().tripname;
  const tripIndex = tripManager.myTrips.findIndex(
    trip => trip.name == tripName
  );
  const spotIndex = tripManager.myTrips[tripIndex].spots.findIndex(
    spot => spot.name == spotName
  );
  const infoWindowHtml = renderer.renderEditSpot(
    tripManager.myTrips[tripIndex].spots[spotIndex]
  );
